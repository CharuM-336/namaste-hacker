...
An **Experience Component** (EC) is a **semantic, behavior‑first primitive** that encapsulates *meaning* (e.g., “chapter card”, “timeline marker”), *state*, *motion*, and *accessibility* without prescribing *how* it looks on any particular platform.  
...
| Appearance | Defined via JSX + style props. | Appearance is a *rendering concern* resolved by the renderer based on theme, device, and motion settings. |
...
- **AI alignment** – The AI already decides *what* the experience should feel like (Experience DNA). ECs give the AI a vocabulary to express *that* feeling.
...
- **World** – Global ambience, theme, progress model; contains *scenes*.
- **Scene** – Logical phase (Arrival, Reading, Reflection…). Owns *zones* for spatial culling.
...
> *All 60 components below follow this exact schema. Only the template is shown once; the full component library (Part 4) is expanded in the appendix.*
...
*The complete, machine‑readable JSON for each component lives in the Appendix.*
...
3. **Parent‑Child Contracts** – A parent module declares a *slot* schema; each child component must satisfy the slot’s required inputs.
4. **Reusable Patterns** –
...
*State transitions are driven by the **Interaction Contracts** (Part 8) and by the **Progress Engine**.*
...
All interactions are **declarative** – they merely describe *what* the user wants; the renderer decides *how* to implement the visual feedback.
...
2. **Module‑to‑Component Mapper** looks up the module `type` in the registry to retrieve a *primary component* (e.g., `type:"timeline"` → `TimelineComponent`).
...
**Example Mapping** – *Harry Potter Blueprint* `module { id:"m-hero", type:"HeroBanner" }` → Component `ArrivalHero` with inputs `title="Welcome to Hogwarts"`, `backgroundMedia="hogwarts_gate.mp4"`, motion `type:"fadeIn"`, state `Idle → Focused` on hover.
...
```
*Key differences*: Heavy **Companion** and **Exploration** components, many map‑related modules.
...
```
*Focus*: **EvidenceBoard**, **CharacterCard**, heavy **Reflection**.
...
```
*Emphasis*: **ConceptGraph**, **KnowledgeCluster**, minimal narrative UI.
...
```
*Focus*: **QuizBlock**, **Flashcard**, **ProgressTracking**.
...
```
*Emphasis*: **InteractiveSimulation**, **StatisticsPanel**, **Reflection**.
...
*Prepared for inclusion in* **`docs/EXPERIENCE_COMPONENT_SYSTEM.md`** *as the definitive technical reference for the Experience Component System.*
